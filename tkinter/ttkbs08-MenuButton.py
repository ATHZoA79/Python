...
item_var = StringVar()
style_list = ['primary', 'secondary', 'danger', 'info', 'outline primary', 'outline secondary', 'outline danger']
for x in style_list:
  # use lambda to pass x into function
  # use x=x to prevent overwrite previous lambda function
  inner_menu.add_radiobutton(label=x, variable=item_var, command=lambda x=x :setLabel(x))
